...
// ── Alertas de oportunidade (C2.4) ────────────────────────────

export interface AlertaDTO {
  id: string
  clienteId: string
  tipo: 'nova_entidade' | 'score_alterado'
  entidadeAlvoId: string
  entidadeNome: string
...
export interface IAlertaRepository {
  ...
  atualizarStatus(id: string, status: AlertaDTO['status']): Promise<AlertaDTO | null>
  existeParaEntidade(clienteId: string, entidadeAlvoId: string): Promise<boolean>
}
